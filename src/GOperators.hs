...
mutateTree (Arithmetic x b1 b2) env =
  (Arithmetic x) <$> (mutate b1 env) <*> (mutate b2 env)
...
across env (Arithmetic a1 ta1 ta2) (Arithmetic _ tb1 tb2) = do
  tb1_ <- ([tb1, tb2] !!) <$> randomRIO (0, 1)
  tb2_ <- ([tb1, tb2] !!) <$> randomRIO (0, 1)
